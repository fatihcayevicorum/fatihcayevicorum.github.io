...
function openPaymentEdit(id){const sale=state.sales.find(s=>s.id===id);if(!sale||isLockedDay(state.selectedDate))return;const bill=number(sale.baseTotal)||billFromItems(sale.items),cash=number(sale.cashAmount),bank=number(sale.transferAmount),type=bank>0&&cash<=0?"transfer":"cash",received=type==="transfer"?bank:cash||bill;state.selectedSaleId=id;$("paymentEditTitle").textContent=sale.title||baseSlotTitle(sale.slotKey);$("paymentEditBill").textContent=money(bill);$("paymentEditType").value=type;$("paymentEditAmount").value=formatInputAmount(received);renderPaymentEditPreview();$("paymentEditDialog").showModal();setTimeout(()=>$("paymentEditAmount").select(),80)}
