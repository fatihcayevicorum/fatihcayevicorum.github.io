...
let catalog={categories:[],items:[]},stocks=[],cart=[],category="all",busy=false,pendingBusinessDate="",stockReady=false,businessDate=today();
...
function renderCart(){const qty=cart.reduce((s,x)=>s+x.quantity,0);cartEmpty.hidden=cart.length>0;items.innerHTML=cart.map(x=>`<article class="order-item"><div><strong>${esc(x.name)}</strong><small>Dahili tüketim${pendingBusinessDate&&pendingBusinessDate!==businessDate?` • ${esc(pendingBusinessDate)}`:""}</small></div><div class="qty"><button type="button" data-internal-id="${esc(x.id)}" data-internal-qty="-1">−</button><b>${x.quantity}</b><button type="button" data-internal-id="${esc(x.id)}" data-internal-qty="1">+</button></div></article>`).join("");total.textContent=qty;save.disabled=!qty||busy}
async function saveConsumption(){
  if(busy||!cart.length)return;if(!stockReady)return notify("Stok verisi henüz hazır değil.");
  busy=true;renderCart();
  try{
    const submittedCart=cart.map(x=>({...x})),submittedNote=note.value.trim(),ref=doc(consumptionCol),date=pendingBusinessDate||businessDate,createdAtMs=Date.now();
    await runTransaction(db,async tx=>{
      ...
      for(const line of submittedCart){
        ...
        for(const stock of links){
          ...
          const fresh=snap.data(),amount=line.quantity*(Number(fresh.deductionAmount)||1),before=Number(fresh.quantity)||0,unitCost=Number(fresh.unitCost)||Number(stock.unitCost)||0;
...
function restorePending(){
  try{
    ...
    if(!saved){localStorage.removeItem(pendingStorageKey);cart=[];note.value="";return}
    pendingBusinessDate=saved.businessDate||businessDate;if(pendingBusinessDate!==businessDate)notify(`Bekleyen dahili tüketim ${pendingBusinessDate} tarihinden korundu.`);cart=Array.isArray(saved.items)?saved.items.map(x=>({id:String(x.id||""),name:String(x.name||"Ürün"),quantity:Math.max(1,Math.floor(Number(x.quantity)||1))})).filter(x=>x.id):[];
    note.value=String(saved.note||"").slice(0,80);
  }catch(error){console.warn("Bekleyen dahili tüketim listesi okunamadı.",error);notify("Bekleyen liste okunamadı; cihazdaki kayıt silinmedi.")}
}
function persistPending(){
  try{
    if(!cart.length&&!note.value.trim()){localStorage.removeItem(pendingStorageKey);pendingBusinessDate="";return}
    localStorage.setItem(pendingStorageKey,JSON.stringify({businessDate:pendingBusinessDate||(pendingBusinessDate=businessDate),items:cart.map(x=>({id:x.id,name:x.name,quantity:x.quantity})),note:note.value.trim(),updatedAtMs:Date.now()}));
