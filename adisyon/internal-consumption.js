...
openButton.addEventListener("click",()=>{cart=[];category="all";search.value="";note.value="";render();dialog.showModal();requestAnimationFrame(()=>{search.focus({preventScroll:true});search.select()})});
...
grid.addEventListener("click",e=>{const b=e.target.closest("[data-internal-product]");if(!b)return;const product=availableProducts().find(x=>x.id===b.dataset.internalProduct);if(!product)return;const line=cart.find(x=>x.id===product.id);line?line.quantity++:cart.push({id:product.id,name:product.name,quantity:1});search.value="";render();requestAnimationFrame(()=>search.focus({preventScroll:true}))});
