...
let categories={income:defaultIncome,expense:defaultExpense},busy=false;
...
async function openQuickCash(){
  ...
  renderCategories();
  $("quickCashDate").textContent=`İşlem tarihi: ${formatDate(today())} • Saat otomatik kaydedilir`;
  dialog.showModal();
...
async function saveQuickMovement(event){
  ...
  try{
    await setDoc(doc(collection(db,"adminCashMovements")),{type,amount:value,account,fromAccount:"",toAccount:"",category:selectedCategory,description:note||selectedCategory,businessDate:today(),automatic:false,source:"pos-quick-cash",createdAtMs,updatedAtMs:createdAtMs,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),createdBy:auth.currentUser.uid});
    dialog.close();
