...
import{collection,doc,getDoc,getFirestore,serverTimestamp,runTransaction,setDoc}from"https://www.gstatic.com/firebasejs/12.16.0/firebase-firestore.js";
...
let categories={income:defaultIncome,expense:defaultExpense},activeBusinessDate=today(),busy=false,canManageCash=false;
...
async function openQuickCash(){
  if(!await hasPanelAccess(auth.currentUser,db,'pos'))return;canManageCash=await hasPanelAccess(auth.currentUser,db,'cash');
  if(canManageCash){lockSensitiveAccess();if(!await requireSensitiveAccess({title:'Hızlı Kasa İşlemi',message:'Yönetici PIN’ini girin.'}))return}
  form.reset();
...
async function loadCategories(){
  if(!canManageCash){categories={income:defaultIncome,expense:defaultExpense};return}
  try{
...
function renderAccountLabel(){
  ...
  if(input)input.value=income?"card":"creditCard";form.querySelectorAll("[name=quickCashAccount]").forEach(x=>x.disabled=!canManageCash&&!income&&x.value!=="cash");if(!canManageCash&&!income)form.elements.quickCashAccount.value="cash";
}
...
async function saveQuickMovement(event){
  ...
  if(!canManageCash&&type==="expense"&&account!=="cash")return toast("Personel giderleri yalnızca nakit kaydedilir.");
  ...
  try{
    await runTransaction(db,async tx=>{const setting=await tx.get(doc(db,"adminAppSettings","pos")),finance=await tx.get(doc(db,"adminFinanceDays",activeBusinessDate));if(setting.data()?.currentBusinessDate!==activeBusinessDate||finance.data()?.locked===true)throw Error("locked");tx.set(doc(collection(db,"adminCashMovements")),{type,amount:value,account,fromAccount:"",toAccount:"",category:selectedCategory,description:note||selectedCategory,businessDate:activeBusinessDate,automatic:false,source:"pos-quick-cash",createdAtMs,updatedAtMs:createdAtMs,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),createdBy:auth.currentUser.uid})});
    dialog.close();
