...
import{deviceLimitMessage,registerDevice}from"./device-access.js";
...
onAuthStateChanged(auth,async user=>{if(!user)return;try{const profile=await getDoc(doc(db,"merchantProfiles",user.uid));if(!profile.exists()||profile.data().active===false)return;await registerDevice(app,"merchant");location.replace("esnaf-paneli/")}catch(error){await signOut(auth);show(deviceLimitMessage(error))}});
form.addEventListener("submit",async event=>{event.preventDefault();setBusy(true);try{const login=normalize(username.value);if(!login)throw Error("invalid");const result=await signInWithEmailAndPassword(auth,`${login}@esnaf.fatihcayevi.local`,password.value);const profile=await getDoc(doc(db,"merchantProfiles",result.user.uid));if(!profile.exists()||profile.data().active===false){await signOut(auth);throw Error("inactive");}await registerDevice(app,"merchant");location.replace("esnaf-paneli/");}catch(error){console.error(error);if(auth.currentUser)await signOut(auth);show(String(error.code||"").includes("resource-exhausted")?deviceLimitMessage(error):error.message==="inactive"?"Hesabınız kullanıma kapalı. İşletmeyle görüşün.":"Kullanıcı adı veya şifre hatalı.");setBusy(false);}});
