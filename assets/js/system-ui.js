...
if(supportsManagementNotifications){
  onAuthStateChanged(auth,async user=>{
    ...
    if(profile){
      ...
      import("./in-app-notifications.js?v=2").then(({installInAppNotifications})=>installInAppNotifications({app,user,canManage:isOwner(user)||profile.permissions?.includes("notifications")})).catch(error=>console.error("Uygulama içi bildirimler başlatılamadı:",error));
    }
  });
}
