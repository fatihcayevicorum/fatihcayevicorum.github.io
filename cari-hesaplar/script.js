...
function renderMovements(){$("movementEmpty").hidden=movements.length>0;$("movementList").innerHTML=movements.map(x=>`<article class="movement-row"><span><strong>${esc(x.title||"Cari hareketi")}</strong><small>${esc(x.note||"")} ${x.createdAtMs?`• ${dateTime(new Date(x.createdAtMs))}`:""}</small></span><b>${money(x.amount)}</b></article>`).join("")}
