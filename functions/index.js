...
exports.submitOwnStaffProfileChange=onCall({region:"europe-west1",cors:true},async request=>{
  const uid=request.auth?.uid;if(!uid)throw new HttpsError("unauthenticated","Oturum açmanız gerekiyor.");if(uid===OWNER_UID)throw new HttpsError("permission-denied","Ana yönetici bilgileri Kullanıcı Yönetimi'nden değiştirilir.");
  const staffRef=db.doc(`staffUsers/${uid}`),staffSnap=await staffRef.get();if(!staffSnap.exists||staffSnap.data().active!==true)throw new HttpsError("permission-denied","Personel hesabı aktif değil.");
  const current=staffSnap.data(),displayName=cleanName(request.data?.displayName),phone=normalizePhone(request.data?.phone),passwordChangeRequested=request.data?.passwordChangeRequested===true,changeRef=db.doc(`staffProfileChangeRequests/${uid}`),existing=await changeRef.get();
  if(existing.exists&&existing.data().status==="pending")throw new HttpsError("failed-precondition","Zaten yönetici onayı bekleyen bir talebiniz var.");
  if(displayName===current.displayName&&phone===current.phone&&!passwordChangeRequested)throw new HttpsError("invalid-argument","Değiştirmek istediğiniz bir bilgi seçin.");
  if(phone!==current.phone){try{const found=await getAuth().getUserByEmail(loginEmail(phone));if(found.uid!==uid)throw new HttpsError("already-exists","Bu telefon numarası başka hesapta kayıtlı.")}catch(error){if(error instanceof HttpsError)throw error;if(error.code!=="auth/user-not-found")throw new HttpsError("internal",error.message)}}
  const now=Date.now(),body=`${current.displayName||"Personel"} kullanıcı bilgilerini güncellemek için onay bekliyor.`;
  const batch=db.batch();batch.set(changeRef,{uid,currentDisplayName:current.displayName||"",currentPhone:current.phone||"",requestedDisplayName:displayName,requestedPhone:phone,passwordChangeRequested,status:"pending",createdAtMs:now,createdAt:FieldValue.serverTimestamp(),requestedBy:uid,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp()});batch.set(db.doc(`${ADMIN_IN_APP_NOTIFICATION_COLLECTION}/staff-profile-change-${uid}`),{type:"staff-profile-change",title:"Personel ayar talebi",body,staffUid:uid,staffName:current.displayName||"Personel",ownerOnly:true,link:"/bildirim-merkezi/#personel-ayar-talepleri",readBy:{},createdAtMs:now,createdAt:FieldValue.serverTimestamp()},{merge:true});await batch.commit();
  sendAdminTeaPush({type:"admin-staff-profile-change",tag:`staff-profile-change-${uid}`,body,link:`${SITE_URL}/bildirim-merkezi/#personel-ayar-talepleri`}).catch(error=>logger.warn("Personel ayar talebi push bildirimi gönderilemedi.",{uid,error:String(error.message||error)}));
  await auditUserAction("staff-profile-change-request",uid,uid,{displayNameChanged:displayName!==current.displayName,phoneChanged:phone!==current.phone,passwordChangeRequested});return{status:"pending"}
});
exports.resolveStaffProfileChange=onCall({region:"europe-west1",cors:true},async request=>{
  requireOwner(request);const uid=String(request.data?.uid||""),action=String(request.data?.action||"");if(!uid||uid===OWNER_UID||!["approve","reject"].includes(action))throw new HttpsError("invalid-argument","Talep bilgisi geçersiz.");
  const changeRef=db.doc(`staffProfileChangeRequests/${uid}`),changeSnap=await changeRef.get();if(!changeSnap.exists||changeSnap.data().status!=="pending")throw new HttpsError("failed-precondition","Bu talep daha önce işleme alınmış.");const change=changeSnap.data(),now=Date.now(),notificationRef=db.doc(`${ADMIN_IN_APP_NOTIFICATION_COLLECTION}/staff-profile-change-${uid}`);
  if(action==="reject"){const batch=db.batch();batch.set(changeRef,{status:"rejected",resolvedAtMs:now,resolvedAt:FieldValue.serverTimestamp(),resolvedBy:request.auth.uid,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp()},{merge:true});batch.update(notificationRef,{[`readBy.${request.auth.uid}`]:true,readAtMs:now,updatedAt:FieldValue.serverTimestamp()});await batch.commit();await auditUserAction("staff-profile-change-reject",uid,request.auth.uid);return{status:"rejected"}}
  const staffRef=db.doc(`staffUsers/${uid}`),staffSnap=await staffRef.get();if(!staffSnap.exists||staffSnap.data().active!==true)throw new HttpsError("failed-precondition","Personel hesabı aktif değil.");
  const displayName=cleanName(change.requestedDisplayName),phone=normalizePhone(change.requestedPhone);try{const found=await getAuth().getUserByEmail(loginEmail(phone));if(found.uid!==uid)throw new HttpsError("already-exists","Bu telefon numarası başka hesapta kayıtlı.")}catch(error){if(error instanceof HttpsError)throw error;if(error.code!=="auth/user-not-found")throw new HttpsError("internal",error.message)}
  await getAuth().updateUser(uid,{email:loginEmail(phone),displayName,disabled:false});const linkedPersonnel=await db.collection("adminPersonnel").where("linkedUserUid","==",uid).get(),passwordChangeRequested=change.passwordChangeRequested===true,passwordAuthorizedUntilMs=passwordChangeRequested?now+24*60*60*1000:0,batch=db.batch();batch.set(staffRef,{displayName,phone,active:true,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp(),updatedBy:request.auth.uid},{merge:true});linkedPersonnel.docs.forEach(item=>batch.set(item.ref,{displayName,phone,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp(),updatedBy:request.auth.uid},{merge:true}));batch.set(changeRef,{status:"approved",approvedAtMs:now,approvedAt:FieldValue.serverTimestamp(),resolvedBy:request.auth.uid,passwordAuthorizedUntilMs,passwordCompleted:!passwordChangeRequested,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp()},{merge:true});batch.update(notificationRef,{[`readBy.${request.auth.uid}`]:true,readAtMs:now,updatedAt:FieldValue.serverTimestamp()});await batch.commit();await auditUserAction("staff-profile-change-approve",uid,request.auth.uid,{displayName,phone,passwordChangeRequested});return{status:"approved",passwordChangeRequested,passwordAuthorizedUntilMs}
});
exports.completeOwnStaffPasswordChange=onCall({region:"europe-west1",cors:true},async request=>{
  const uid=request.auth?.uid;if(!uid||uid===OWNER_UID)throw new HttpsError("permission-denied","Bu işlem personel hesabına özeldir.");const changeRef=db.doc(`staffProfileChangeRequests/${uid}`),changeSnap=await changeRef.get(),change=changeSnap.data()||{},now=Date.now();if(!changeSnap.exists||change.status!=="approved"||change.passwordChangeRequested!==true||change.passwordCompleted===true||Number(change.passwordAuthorizedUntilMs)<=now)throw new HttpsError("failed-precondition","Şifre değiştirme onayı bulunamadı veya süresi doldu.");const password=cleanPassword(request.data?.password);await getAuth().updateUser(uid,{password});await changeRef.set({status:"completed",passwordCompleted:true,passwordCompletedAtMs:now,passwordCompletedAt:FieldValue.serverTimestamp(),updatedAtMs:now,updatedAt:FieldValue.serverTimestamp()},{merge:true});await auditUserAction("staff-password-change-complete",uid,uid);return{status:"completed"}
});
exports.configureOwnerLogin=onCall({region:"europe-west1",cors:true},async request=>{
  requireOwner(request);const phone=normalizePhone(request.data?.phone),password=cleanPassword(request.data?.password);try{await getAuth().updateUser(OWNER_UID,{email:loginEmail(phone),password,displayName:"Fatih Ali Altınlı",disabled:false});await db.doc(`staffUsers/${OWNER_UID}`).set({displayName:"Fatih Ali Altınlı",phone,role:"owner",active:true,permissions:PANEL_IDS.filter(id=>id!=="personnel"),permissionSchemaVersion:"r286",deviceLimit:7,updatedAt:FieldValue.serverTimestamp(),updatedBy:OWNER_UID},{merge:true});await auditUserAction("owner-phone-login",OWNER_UID,OWNER_UID,{phone});return{phone}}catch(error){if(error.code==="auth/email-already-exists")throw new HttpsError("already-exists","Bu telefon numarası başka hesapta kayıtlı.");throw new HttpsError("internal",error.message)}
});
...
async function ownerReminderPreferences(){return(await db.doc(`adminReminderPreferences/${OWNER_UID}`).get()).data()||{}}
async function notificationDocsByTypes(types=[]){if(!types.length)return[];const snapshots=await Promise.all(types.map(type=>db.collection(ADMIN_IN_APP_NOTIFICATION_COLLECTION).where("type","==",type).get()));return snapshots.flatMap(snapshot=>snapshot.docs)}
async function deleteNotificationDocs(documents=[]){if(!documents.length)return;const writer=db.bulkWriter();documents.forEach(item=>writer.delete(item.ref));await writer.close()}
async function clearBusinessNotifications(types=[]){await deleteNotificationDocs(await notificationDocsByTypes(types))}
async function stockInOpenPurchaseFlow(stockItemId){
  const[draftSnap,ordersSnap]=await Promise.all([db.doc("adminPurchaseDrafts/notification-draft").get(),db.collection("adminPurchaseOrders").where("status","in",["pending","partial"]).get()]);
...
exports.syncPurchaseOrderNotifications=onDocumentWritten({document:"adminPurchaseOrders/{orderId}",region:"europe-west1"},async event=>{
  const after=event.data?.after.data()||{},pendingItems=Array.isArray(after.items)?after.items.filter(item=>(Number(item.receivedQuantity)||0)<(Number(item.orderedQuantity)||0)):[],oldReminders=await notificationDocsByTypes(["purchase-order-stale"]),deletes=oldReminders.filter(item=>String(item.data()?.sourceId||"")===String(event.params.orderId));
  for(const item of pendingItems){const id=String(item.stockItemId||"");if(!id)continue;deletes.push({ref:db.doc(`${ADMIN_IN_APP_NOTIFICATION_COLLECTION}/stock-critical-${id}`)});deletes.push({ref:db.doc(`${ADMIN_IN_APP_NOTIFICATION_COLLECTION}/stock-empty-${id}`)})}
  await deleteNotificationDocs(deletes)
});
...
exports.checkAdminBusinessReminders=onSchedule({schedule:"every 30 minutes",region:"europe-west1",timeZone:"Europe/Istanbul"},async()=>{
  const now=Date.now(),local=localDate(),writes=[],preferences=await ownerReminderPreferences();
  const existingOrderNotifications=await notificationDocsByTypes(["purchase-order-stale"]),keptOrderNotificationIds=new Set;
  if(preferences.purchaseOrders===false)writes.push(deleteNotificationDocs(existingOrderNotifications));else{
    const orders=await db.collection("adminPurchaseOrders").where("status","in",["pending","partial"]).get();
    for(const s of orders.docs){const x=s.data(),pendingItems=(x.items||[]).filter(i=>Number(i.receivedQuantity)<Number(i.orderedQuantity));if(!pendingItems.length){writes.push(s.ref.set({status:"completed",totalReceived:Number(x.totalOrdered)||Number(x.totalReceived)||0,updatedAtMs:now,updatedAt:FieldValue.serverTimestamp()},{merge:true}));continue}const last=Number(x.updatedAtMs)||Number(x.lastReceivedAtMs)||Number(x.createdAtMs)||now;if(now-last<48*60*60*1000)continue;const notificationId=`purchase-order-stale-${s.id}`,date=new Date(Number(x.createdAtMs)||last).toLocaleDateString("tr-TR");keptOrderNotificationIds.add(notificationId);writes.push(putBusinessNotification(notificationId,{type:"purchase-order-stale",preferenceKey:"purchaseOrders",sourceId:s.id,title:"Sipariş listenizi unutmuş olabilirsiniz",body:`${date} tarihli sipariş listenizde ${pendingItems.length} ürün hâlâ bekliyor.`,link:`/siparis-listesi/?order=${encodeURIComponent(s.id)}`}))}
    writes.push(deleteNotificationDocs(existingOrderNotifications.filter(item=>!keptOrderNotificationIds.has(item.id))))
  }
