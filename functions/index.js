...
exports.readSystemBackup=onCall({region:"europe-west1",cors:true,memory:"256MiB"},async request=>{
  ...
  const[meta]=await file.getMetadata(),size=Number(meta.size)||0;if(size>25*1024*1024)throw new HttpsError("resource-exhausted","Yedek doğrudan açmak için çok büyük.");
  if(request.data?.chunked===true){const offset=Number(request.data.offset||0),generation=String(meta.generation);if(!Number.isSafeInteger(offset)||offset<0||offset>=size)throw new HttpsError('invalid-argument','Yedek parçası geçersiz.');if(request.data.generation&&String(request.data.generation)!==generation)throw new HttpsError('failed-precondition','Yedek dosyası değişti.');const end=Math.min(size,offset+3*1024*1024),version=getStorage().bucket().file(`system-backups/${name}`,{generation}),[bytes]=await version.download({start:offset,end:end-1});return{name,base64:bytes.toString('base64'),offset,nextOffset:end,size,generation}}
  if(size>8*1024*1024)throw new HttpsError('resource-exhausted','Bu yedeği açmak için uygulamayı güncelleyin.');const[bytes]=await file.download();return{name,text:bytes.toString("utf8"),size};
});
...
exports.closeBusinessDay=onCall({region:'europe-west1',cors:true},async request=>{
 requireOwner(request);const date=String(request.data?.businessDate||''),start=Number(request.data?.businessDayStartedAtMs);if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!Number.isSafeInteger(start)||start<=0)throw new HttpsError('invalid-argument','İş günü geçersiz.');
 return db.runTransaction(async tx=>{
  const settingsRef=db.doc('adminAppSettings/pos'),closingRef=db.doc(`adminDailyClosings/${date}-${start}`);
  const [settings,closing,tea,open,saleDocs,cashDocs]=await Promise.all([tx.get(settingsRef),tx.get(closingRef),tx.get(db.doc('adminTea/state')),tx.get(db.collection('adminOrders').where('status','==','open')),tx.get(db.collection('adminSales').where('businessDate','==',date)),tx.get(db.collection('adminCashMovements').where('businessDate','==',date))]);
  const current=settings.data()||{};if(closing.exists||current.currentBusinessDate!==date||Number(current.currentBusinessDayStartedAtMs)!==start)throw new HttpsError('failed-precondition','İş günü değişmiş veya zaten kapatılmış.');if(!open.empty)throw new HttpsError('failed-precondition','Açık adisyonlar var. Önce adisyonları kapatın.');const teaState={activeBrews:[],history:[],...tea.data()};if(teaState.activeBrews.length)throw new HttpsError('failed-precondition','Aktif demlikleri bitirin.');
  const report=require('./day-report.cjs')({date,businessDayStartedAtMs:start,teaState,sales:saleDocs.docs.map(x=>({id:x.id,...x.data()})),cashMovements:cashDocs.docs.map(x=>({id:x.id,...x.data()}))});
  const stable=x=>Array.isArray(x)?x.map(stable):x&&typeof x==='object'?Object.fromEntries(Object.keys(x).sort().map(k=>[k,stable(x[k])])):x;
  if(JSON.stringify(stable(report))!==JSON.stringify(stable(request.data?.expectedReport||{})))return{closed:false,report};
  const now=Date.now(),nextDate=new Date(date+'T12:00:00Z');nextDate.setUTCDate(nextDate.getUTCDate()+1);const next=nextDate.toISOString().slice(0,10);
  tx.set(closingRef,{...report,accountingMode:'report-only',cashTransferDisabled:true,cashTransferNote:'Para hareketleri işlem anında kaydedilir. Gün kapatma rapor ve iş günü kilidi oluşturur.',previousLastClosedDate:current.lastClosedDate||'',closedAtMs:now,closedAt:FieldValue.serverTimestamp(),closedBy:request.auth.uid,nextBusinessDate:next});tx.set(settingsRef,{lastClosedDate:date,currentBusinessDate:next,currentBusinessDayStartedAtMs:now,updatedAt:FieldValue.serverTimestamp()},{merge:true});return{closed:true,nextBusinessDate:next,startedAtMs:now,report};
 });
});
