...
function showNewOrder(order,total){if(!notificationsEnabled())return;const business=order.businessName||order.merchantName||"Esnaf",quantity=Number(order.quantity)||0,title=total>1?`${total} yeni esnaf siparişi`:`${business} Çay söyledi`,text=total>1?"Siparişleri görmek için dokunun.":`${quantity} Çay${order.note?` • ${order.note}`:""}`;showAlertPopup(title,text);playBell();vibrate();showSystemNotification(title,text)}
