...
function render(){
  ...
  const worked=monthAttendance.filter(x=>x.status==="worked"),earned=worked.reduce((sum,x)=>sum+number(x.wageSnapshot),0),paid=monthPayments.reduce((sum,x)=>sum+number(x.amount),0);
...
function renderCalendar(person){
  ...
  for(let day=1;day<=days;day++){const date=`${selectedMonth}-${String(day).padStart(2,"0")}`,row=rows.get(date),future=date>today(),label={worked:"Çalıştı",leave:"İzinli",absent:"Gelmedi"}[row?.status]||"Kayıt yok";cells.push(`<button class="calendar-day ${row?.status||""} ${date===today()?"today":""} ${future?"future":""}" type="button" data-attendance-date="${date}" ${future?"disabled":""}><b>${day}</b><small>${label}</small></button>`)}
...
async function savePayment(e){
  ...
  if(!person)return;if(!businessDate||businessDate>today()||businessDate<FINANCE_START_DATE)return message("paymentMessage","Geçerli bir ödeme tarihi seçin.");if(isFinanceLocked(businessDate)||existing&&isFinanceLocked(existing.businessDate))return message("paymentMessage","Finans günü kapalı olan ödeme değiştirilemez.");if(amount<=0)return message("paymentMessage","Geçerli bir ödeme tutarı girin.");
